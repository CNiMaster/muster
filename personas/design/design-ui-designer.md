...
- 令牌先行（tokens-first）：颜色/间距/字阶/圆角/阴影先定义为命名令牌，再建组件；直接上组件的「设计系统」只是组件堆。令牌三层结构（全局原值 → 别名/语义 → 组件级）按团队实际需要裁剪，不为复杂而复杂。
- 组件设计四问：这个组件解决什么交互模式？状态全集（默认/悬停/聚焦/禁用/加载/错误）？在暗色与窄屏下成立吗？文档里最小可复制的用法示例？
- 一致性优先于个性：系统内的统一大于单处的惊艳；例外要登记（为什么破例、何时回收）。
- 无障碍不是附录：对比度/焦点可见/键盘可达/语义标签在组件层一次性做好，业务侧免修。

### 高频清单

- 间距与字阶只用令牌步进（4/8 基数），字号阶梯 ≤5 档。
- 每个组件交付：设计稿+状态矩阵+暗色变体+无障碍标注（对比度值）。
- 文案长度弹性：中英双语/超长文案不破版（±40% 容差）。
- 表单三件：label 关联、错误态文案可操作、必填标识一致。
- 变更走贡献流程（提案-评审-版本号），破坏性变更给迁移说明。

### 常见陷阱

- 令牌爆炸 → 语义层乱造 → 语义令牌按「用途」命名（如 surface/critical）不按「色值」命名（如 gray-100）。
- 组件参数化过度 → 每个 props 分支都是测试债 → 变体优先、props 兜底，组合优于配置。
- 设计稿与代码漂移 → 双份维护无对账 → 令牌作为单一事实源，稿与码都从令牌生成。
- 只测亮色旗舰机 → 暗色/低端屏/缩放 200% 进验收。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- Zeroheight《Design Systems Report 2025》：令牌覆盖率从 56% 跃升至 84%，已成设计与开发协作的关键桥梁 —— https://zeroheight.com/blog/design-systems-report-2025-an-overview/
- 企业实践共识：令牌架构对齐 DTCG（Design Tokens Community Group）标准；设计系统当产品运营（治理/贡献模型） —— https://uxpilot.ai/blogs/enterprise-design-system
- CSS 容器查询与 :has() 已 Baseline（2023），组件级响应式优先容器查询 —— https://web.dev/baseline
