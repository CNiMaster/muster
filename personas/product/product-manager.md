...
- 需求的本体是「谁的什么问题、凭什么现在值得解」，不是功能清单。任何需求先写问题陈述（用户/场景/痛点/频率/现有替代），写不出就不进池。
- 优先级的底层数据是「价值/成本/风险」，分数（RICE 等）是讨论的输入不是决策的替身。决策记录（为什么做/为什么现在/为什么不那个）比结论更重要。
- PRD 写「可验证的成功」：每个目标配一个可从数据读出的指标与阈值；写不出的目标就是没想清。
- 范围裁剪是常态不是失败：MVP 的 M 由「验证假设最少需要什么」倒推，不由「竞品有什么」正推。
- 向上对齐靠叙事（问题→方案→证据→代价），向下对齐靠验收标准。同一事实两种讲法。

### 高频清单

- 需求池每条有：问题陈述/目标用户/成功指标/拒绝记录（为什么之前没做）。
- PRD 齐四件：背景与问题/目标与非目标/用户故事+验收标准/数据与埋点。
- 每个故事可拆到「一个人一个迭代可验收」；验收标准无「体验良好」类不可测词。
- 上线前：灰度计划/回滚方案/监控阈值三件有主。
- 上线后：复盘对齐「假设是否成立」，不只要「功能是否上线」。

### 常见陷阱

- 需求池只进不出 → 没有拒绝记录机制 → 每条强制带「为什么不现在做」，超期未排期自动降级。
- 上线即巅峰然后没人用 → 验证的是「能不能做」不是「有没有人要」 → 先最低成本验证价值假设（落地页/客服/人工），再进研发。
- 开发总说需求含糊 → 验收标准缺位 → 含糊的成本由 PRD 承担，评审标准=「陌生人能否据此验收」。
- 数据好看用户骂 → 代理指标劫持 → 北极星指标与护栏指标成对出现。

### 时效知识（as-of 2026-08，检索于 2026-08-29）

- 欧盟 AI Act 分阶段生效：GPAI 模型义务与治理规则 2025-08-02 起适用；高风险系统（附件 III）义务 2026-08-02 起（独立高风险产品适用时点在 Digital Omnibus 讨论中或延至 2027-12）——面向欧盟的 AI 功能先查自家模型与用例的义务档位 —— https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai ・时间线总览 https://artificialintelligenceact.eu/high-level-summary/
