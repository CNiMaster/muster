...
- 平台策略用决策树不用信仰：团队规模/原生模块需求/性能敏感度/发布节奏四问定跨平台（RN/Flutter）还是双原生；中小团队默认跨平台+关键模块原生桥接。
- 原生感是可验收的清单不是玄学：转场动画跟手、列表滚动不掉帧、手势不吞系统手势、键盘避让正确、深色模式完整——逐条过，不是「感觉像」。
- 性能预算前置：启动时长/帧率/包体/内存四项写进验收；超预算的优化在发布前做，不留给用户投诉。
- 发布通道即工程：OTA 热更边界、灰度分阶段、回滚开关在设计期确定；移动端的发布不是终点是运营起点。

### 高频清单

- 适配三件：安全区（刘海/手势条）、多分辨率资源、横竖屏与平板布局抽查。
- 网络三态（无网/弱网/超时）有明确 UI 与重试策略；本地缓存有失效与清理策略。
- 权限申请前有解释页（为什么需要），拒绝后有降级路径。
- 埋点覆盖关键漏斗，崩溃上报+符号表上传是发布流水线的一环。
- 深链与推送冷启动路径单独测试。

### 常见陷阱

- JS 线程卡顿拖垮动画 → 重计算/同步桥调用占用主线程 → 重活下沉（worklet/原生模块），列表虚拟化。
- 升级后布局崩 → 依赖旧架构行为 → 锁版本+升级专项回归（新架构下旧 workaround 反而有害，迁移时删干净）。
- 热更越过应用商店审核红线 → 动态下发可执行逻辑 → 热更只发资源与配置级变更，遵守平台政策。
- 真机差异（厂商 ROM/权限管控）漏测 → 只测旗舰机 → 低端机+主流国产 ROM 进常规回归池。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- React Native 0.82（2025-10）起旧架构（Bridge）被移除，新架构（Fabric+TurboModules+JSI）成为唯一形态；0.83（2025-12）继续清理兼容层——旧项目官方建议先迁 0.81/Expo SDK 54 再升级 —— https://reactnative.dev/blog/2025/10/08/react-native-0.82
- Expo SDK 55（RN 0.83+React 19.2）新架构恒开且不可关闭、newArchEnabled 标志删除；SDK 54 起 React Compiler（自动 memoization）默认开启；Expo+EAS 已是新项目事实上的默认起点 —— https://docs.expo.dev/guides/new-architecture/
