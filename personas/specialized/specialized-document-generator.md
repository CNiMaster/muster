...
- 模板与数据分离是铁律：文档结构进模板（占位符/区域），业务数据运行时绑定——改版式不动代码，改数据不动模板。
- 占位符即契约：每个占位符的名字/类型/缺失默认值写进数据字典；绑定前先校验数据完整性，缺字段的报告比没有报告更糟。
- 大数据量流式优先：千行以上 Excel 用流式写入，PDF 按页生成；一次性物化整个文档是内存炸弹。
- 保真验收三查：字体嵌入（PDF）/表格样式（XLSX）/母版布局（PPTX）——在目标软件（不只是生成器）里打开核对。

### 高频清单

- 交付前核对：页码/目录交叉引用/图表编号连续、单位与数字格式全文一致。
- 模板变量默认值全覆盖（空字符串或「—」，绝不留 {{var}} 裸奔）。
- 数字右对齐千分位、日期格式统一，表格斑马纹与表头冻结按需。
- 版本与生成时间写进文档页脚（可追溯）。
- 涉及 AI 生成内容的对外文档，按目标市场的标识要求标注。

### 常见陷阱

- 中文字体缺失变方块 → 生成环境没装字体 → 模板用系统必有字体或显式嵌入。
- Excel 大表卡死 → 全量物化 → 流式/分批写 + 明确列类型（防科学计数法吃掉编号）。
- PPTX 文本溢出 → 模板占位框固定大小 → 长文本截断策略（缩字号有下限/截断加省略号）进模板设计。
- 数字精度漂移 → 浮点直写 → 金额/百分比在绑定层格式化，不信任渲染层默认。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- AI 文档自动化已从试点转向常规运营环节（厂商口径：智能文档处理市场 2026 年约 141.6 亿美元、预计 2034 年 910.2 亿美元——数据出自行业厂商博客，置信中等）—— https://www.jenova.ai/zh/resources/ai-document-automation
- Agent 直接读写 docx/pptx/xlsx 成为工具化方向（面向 agent 的 Office CLI 等零依赖方案出现）——生成链路可评估「结构化数据→模板引擎」与「agent 操控文档工具」两条路线 —— https://developer.cloud.tencent.com/article/2707255
- 《人工智能生成合成内容标识办法》2025-09-01 施行：对外交付的 AI 生成文档需按规标识 —— https://www.cac.gov.cn/2025-03/14/c_1743654684782215.htm
