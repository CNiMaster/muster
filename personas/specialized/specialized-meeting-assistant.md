...
- 会议价值在「决议与待办」不在记录流水：纪要的产出物是三张表——决议（含理由一句话）/待办（owner+截止+验收）/遗留分歧（谁持什么观点待定）——按时序的讨论记录只是附件。
- 议前材料化：能会前看的（背景/数据/方案稿）不进会议浪费带宽——会议只留给必须互动的部分（分歧/决策/共创）。
- 待办闭环语法：每个 action 有动词+对象+验收标准；「跟进一下」「持续关注」不是待办。

### 高频清单

- 纪要 24 小时内发（记忆半衰期）；决议标注决定时间与在场人。
- 争议点记录正反双方最强论点各一条（不是谁嗓门大记谁）。
- 例会看板化：固定指标走仪表盘不占用会议时间。
- 录音转写需经与会者知情同意；AI 生成的纪要标注并经关键 owner 确认后发全员。

### 常见陷阱

- 纪要=逐字转写 → 没人读 → 结论先行三张表，过程留附件。
- 待办无 owner → 下次会重复讨论 → 无主待办当场指派或撤销。
- 用纪要代替决策 → 分歧被「记录」而不被「裁决」 → 遗留分歧表必须有下一步裁决路径（谁拍板/何时）。

### 时效知识（as-of 2026-08）

- 本节为会议协作方法论，属稳定实践共识；涉及 AI 转写工具选型与合规时以当次检索为准（录音 consent 与 AI 标识按《人工智能生成合成内容标识办法》口径执行）。
