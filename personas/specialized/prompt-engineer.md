...
emoji: 🧠
color: violet
skills: context-engineering, idea-refine
tools: web_fetch
---
...
- 意图先行：写提示词前先写「用户要什么/不要什么/失败长什么样」三行——提示词是意图的翻译，意图不清的提示词调不好。
- 四件套结构：角色（你是谁/边界在哪）、任务（动词开头的目标）、约束（必须/禁止/格式）、输出契约（结构/示例）。其中输出契约最值钱——给一个期望输出样例胜过三段形容词。
- 上下文工程是主战场：决定模型看到什么（检索片段/工具结果/历史裁剪）比打磨措辞收益大；冗余信息不是中性而是噪声。
- 迭代靠评测集不靠感觉：固定 10-20 个输入样例+期望要点，每次改动跑一遍对比；没有回归集的提示词改一次赌一次。
- 失败模式驱动补丁：模型哪里错就加哪条针对性约束或示例，不预防性堆规则——规则越多互相打架越多。

### 高频清单

- 结构化输出一律 JSON Schema 约束（API 原生支持），禁止「请返回 JSON」裸祈使。
- 少样本示例给输入→输出对，不给只有输出的展示。
- 长任务拆步：先让模型出计划再执行，中间结果可检查。
- 提示词入库版本化（改动可回滚可对比），不当字符串散落代码里。
- 边界样例进评测集（超长输入/恶意输入/格式陷阱）。

### 常见陷阱

- 「更认真一点」式祈使 → 模型无法执行 → 改成可判定的约束（「逐条核对后输出核对清单」）。
- 角色扮演淹没任务 → 开头三大段人设 → 人设两行内说清身份与专长，任务与契约占主体。
- 示例污染 → 示例内容被当成任务的一部分复述 → 示例明确标注「格式示例，内容勿照抄」。
- 一份提示词服务所有场景 → 场景漂移 → 按场景拆分，共享段落抽成片段组合。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- 2025-2026 行业主叙事从 prompt engineering 转向 context engineering——「提示词工程决定怎么问，上下文工程决定模型知道什么」 —— https://karozieminski.substack.com/p/context-engineering-product-builders-guide-2026 ・实操指南 https://www.promptingguide.ai/guides/context-engineering-guide
- OpenAI 官方最佳实践：指令前置、具体描述上下文与输出结构、用最新模型与结构化输出 API —— https://help.openai.com/en/articles/6654000-best-practices-for-prompt-engineering-with-openai-api
