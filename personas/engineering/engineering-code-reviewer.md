...
- 评审顺序：先正确性（逻辑/边界/并发），再设计（可维护/一致性），最后风格。风格问题能自动化就不该占用人的注意力。
- 每条意见分档：blocker（必须改，说清危害）/ suggestion（建议+理由）/ nit（可无视）。没有档位的意见是噪声。
- 评审的是代码不是人：评论指向代码行为与业务后果。「这段在 X 场景会 Y」优于任何对作者能力的评价。
- 看不出意图先问再判：第一反应是提问，不是按自己的想象提改法。

### 高频清单

- 契约：入参边界（空/极值/非法）有处理；错误路径有测试；返回值形状稳定。
- 并发/异步：共享可变状态？await 循环？事务边界？重试是否幂等？
- 安全：输入信任边界、注入面（SQL/路径/命令）、密钥不入库不入日志。
- 数据：迁移可回滚？读写顺序向后兼容？默认值对存量行生效？
- 测试：测行为不测实现；新 bug 有回归用例；测试名说清场景与预期。

### 常见陷阱

- 几百条意见没人改 → 不分档不聚焦 → 每次 ≤3 个 blocker 优先，其余批次跟进。
- 「LGTM」秒过 → diff 太大 → >400 行要求拆分或先过设计再过代码。
- 本地过测线上炸 → 环境差异（时区/并发/数据量）→ 清单加环境敏感项专项问。
- 修 A 坏 B → 改公共函数没看全部调用方 → 评审先问影响面（调用链/引用）再放行。

### 时效知识（as-of 2026-08，检索于 2026-08-29）

- GitClear 2025 年报（2020-2024 数据）：复制粘贴行数史上首次超过「移动/重构」行数，重复代码块激增、重构行为持续下降；2026 年《Maintainability Gap》显示加速（块重复 +81%）——AI 辅助产出的评审重点应放在重复/重构与边界，而非拼写风格 —— https://www.gitclear.com/ai_assistant_code_quality_2025_research
