...
- 搜索意图先于关键词：每个目标词先分类意图（信息/导航/交易/比较），内容形态匹配意图——意图错配排名再高也无转化。
- 主题集群架构：一个支柱页（全景）+一群集群页（子题）内链成网——站内结构是排名的地基，单页优化是装修。
- E-E-A-T 落到证据：经验/专业/权威/可信不是口号而是可核查信号（作者履历/出处引用/原创数据/更新日期）。
- 技术面三基础：可抓取（robots/站图）、可索引（规范标签/无重复）、快（Core Web Vitals——LCP≤2.5s、INP≤200ms、CLS≤0.1）。

### 高频清单

- 关键词地图季度更新：核心词/长尾词/竞品词三表，标注意图与优先级。
- 每页 Title/Description 独立且含关键词自然出现，结构化数据（FAQ/HowTo/产品）按类型部署。
- 内链审计：孤儿页/断链/过度集中在季度技术体检清一遍。
- 内容刷新优先于新增：既有排名页的更新（数据/年份/新增小节）ROI 高于新页。

### 常见陷阱

- 关键词堆砌 → 判定操纵 → 同义词与语义变体自然分布，密度不是目标。
- 只盯排名不盯 AI 可见度 → 新流量入口流失 → 监测品牌在 AI 回答中的引用份额。
- 外链数量崇拜 → 低质链接反噬 → 相关性与权威性优先。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- AI Overviews 冲击自然流量：同行评审研究（4650 万+样本）显示来源页自然流量平均 -15%，信息类内容 -20~40%——策略从「排名」转向「全域搜索可见度」（排名+AI 引用份额）—— https://authoritytech.io/blog/google-ai-overviews-impact-seo-2026 ・https://eseospace.com/blog/how-ai-overviews-impact-seo-2026/
- GEO（生成式引擎优化）成为新战场：结构化数据、实体图谱、E-E-A-T、权威引用是提升 AI 引用的核心抓手；SEO 未死而是与 GEO 双轨并行 —— https://www.bpms.cn/baike/133.html ・https://zhuanlan.zhihu.com/p/2005955554099622541（信通院口径：2026 国内 GEO 市场 350 亿、渗透率 71%，行业数据置信中等）
