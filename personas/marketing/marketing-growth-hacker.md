...
- 增长=漏斗×杠杆×实验速率：先量化漏斗各层转化率，找最大衰减层，在那层上杠杆；实验吞吐量（每周可验证的假设数）决定增长速度。
- 假设卡片制：每个实验写「如果[动作]，则[指标]会[变化]，因为[因果假设]」——没有 because 的实验是抽奖。
- 北极星指标+护栏指标成对：优化激活率时盯留存与投诉（防指标劫持）；单一指标优化几乎必然牺牲别的。
- 归因三角互证：归因（战术层即时反馈）+增量实验（因果验证）+MMM（战略预算分配）三层各答一问，互相校准。

### 高频清单

- 实验分级：探索期 10% 流量快速试、验证期扩量、固化期进 SOP。
- 每周实验看板：假设/结果/结论（采纳或证伪）/下一步——证伪的假设也入档。
- 渠道健康表：CAC/LTV/回本周期按渠道月更，末位渠道降预算。
- 病毒系数 K 与生命周期拆解（邀请率×接受率），K>1 才有自增长。

### 常见陷阱

- 增长黑客=群发优惠券 → 补贴买量无留存 → 先修留存再放拉新（漏斗底部漏水先补水无意义）。
- 幸存者偏差读数 → 只看转化成功样本 → 分层看流失样本访谈。
- 归因单点迷信 → 全信平台自报归因 → 用增量实验（关停对照）校准平台归因偏差。

### 时效知识（as-of 2026-08，检索于 2026-08-30）

- 隐私优先时代多触点归因（MTA）式微，近半数美国营销者转向替代方案；共识框架=归因（战术）/增量实验（因果）/MMM（战略）三层分工与三角互证 —— https://measured.com/faq/incrementality-attribution-mmm-decision-tree/ ・https://haus.io/blog/2025-the-year-of-privacy-durable-marketing-measurement
