...
- 一篇内容一个转化目标：先写「读完要读者发生什么」（关注/点击/信任/购买），所有段落服务该目标——目标超过一个等于没有。
- 钩子金字塔：标题（3 秒留住）→首段（30 秒给价值预告）→小标题（扫读可懂）——三层各自独立成立，读者停在哪层都有收获。
- 素材先于观点：每个论点配证据（数据/案例/引语），无证据的论点删或降级为提问。
- 平台母语写作：同一主题在小红书（个人体验体）、公众号（结构长文）、抖音脚本（口播+字幕钩子）是三个作品，不是一稿三发。

### 高频清单

- 标题备三选一（利益型/悬念型/数字型），发布后 2 小时数据差的换标题。
- 每篇校对三遍分目的：结构遍/句子遍/校字遍（三遍不混做）。
- 引用数据带出处与年份；AI 辅助内容按平台标识规范标注。
- 交付附带元信息：目标人群/关键词/建议发布位。

### 常见陷阱

- 自嗨式专业 → 术语墙劝退 → 术语首次出现即解释或换白话。
- 信息密度过低注水 → 完读率崩 → 删到「每段都有增量」为止。
- 一稿多发水土不服 → 复用内容重写钩子与结构再发。

### 时效知识（as-of 2026-08，检索于 2026-08-29）

- 《人工智能生成合成内容标识办法》2025-09-01 施行：AI 辅助生成的图文内容发布需按规标识 —— https://www.cac.gov.cn/2025-03/14/c_1743654684782215.htm
