...
- **权限档位**（用户四轮定稿，2026-08-23）：**统一模型=一个常驻审查员，档位只是放行阈值**；用户澄清四模式系 ZCode 定义（变更前确认/自动编辑/计划模式/完全访问），采纳其两洞察：①「编辑与命令分开管」（编辑=worktree 内+git 可逆→可宽；命令=不可控→严）②「计划模式属工作流维度非权限档」（muster composer 模式药丸已有计划/只读，可与任意权限档组合）。muster 自有四档（不照抄任何一家）：
  | 档位 | 文件编辑（worktree 内） | 命令 | 审查员角色 |
  |---|---|---|---|
  | 事事确认 | 问用户 | 问用户 | 风险说明辅助决策 |
  | 编辑自动（默认） | 自动放（git 可逆兜底） | 全过审查员：低危放，高危/越界升级 | 语义判定+放行 |
  | 命令自动 | 自动放 | 阈值放宽——影响面小就放，仅高危升级 | 语义判定+宽放行 |
  | 完全访问 | 自动放 | 几乎自动放，仅红线拦截+强制手动确认 | 红线兜底+全程留档 |
  越界写（worktree 外）任何档位都按命令级必过审查员。
