...
- **权限档位**（用户五轮定稿，2026-08-23）：**统一模型=一个常驻审查员，档位只是放行阈值**；用户澄清四模式系 ZCode 定义（变更前确认/自动编辑/计划模式/完全访问），采纳其两洞察：①「编辑与命令分开管」（编辑=worktree 内+git 可逆→可宽；命令=不可控→严）②「计划模式属工作流维度非权限档」（muster composer 模式药丸已有计划/只读，可与任意权限档组合）；用户砍掉「命令自动/完全访问」的无感差异合并为**三档**（muster 自有，不照抄任何一家）：
  | 档位 | 文件编辑（worktree 内） | 命令 | 审查员角色 |
  |---|---|---|---|
  | 事事确认 | 问用户 | 问用户 | 风险说明辅助决策 |
  | 编辑自动（默认） | 自动放（git 可逆兜底） | 全过审查员：低危放，高危/越界升级 | 语义判定+放行 |
  | 自动执行 | 自动放 | 项目内影响小自动放；**超出项目目录/影响可能外溢/红线级必 AI 判定**，判危险→风险说明卡强制手动确认 | 外溢重点盯防+全程留档 |
