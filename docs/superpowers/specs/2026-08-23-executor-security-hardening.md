...
- **权限档位**（用户三轮定稿，2026-08-23）：**统一模型=一个常驻审查员，档位只是放行阈值**（用户指出原 ai-review「有审查」/permissive「无审查」两套机制的表述是误导——对，本质相同，严格度不同；审查员常驻所有档位，连完全访问也有红线兜底）。四档沿用用户心智命名：
  | 档位 | worktree 内 | 越界命令 | 审查员角色 |
  |---|---|---|---|
  | 变更前确认 | 变更也问用户 | 都问 | 风险说明辅助决策 |
  | 自动编辑（默认） | 自动放 | 过审查员，拿不准升级 | 语义判定+放行 |
  | 继续模式 | 自动放（含已授权目录） | 阈值放宽，高危才升级 | 语义判定+宽放行 |
  | 完全访问 | 自动放 | 几乎自动放，仅红线拦截+全程留档 | 红线兜底+旁观记录 |
