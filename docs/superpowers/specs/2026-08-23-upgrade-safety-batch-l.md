...
- **复审轮（八类镜头，R1-R4）**：R1（Important）快照三件套裸拷贝→**VACUUM INTO**（SQLite 官方在线备份原语：单文件自洽、免 wal/shm 配对脆性；恢复=拷回一个文件+删原位 -wal/-shm）；R2（Minor）空间预算裁剪只累计保留集字节（原实现把已判删目录也计入→超预算多删旧快照；最新一份永远保留）；R3（Minor）contextGovernance keepRecent 夹紧 ≤ maxMessages-4（否则空摘要+消息数不降反涨）；R4（记录不改）executeCleanup 事务内 gzip 归档持锁=低频用户触发可接受、崩溃中间态孤儿归档幂等重跑收敛。
