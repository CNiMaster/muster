...
## 2026-07-15 Schema 驱动公司蓝图平台

本轮把“行业模板”从四组前端固定选项升级为可生成、可校验、可确认、可追溯的通用公司能力包：

- [x] 统一 `CompanyTemplateDraft`/`CompanyTemplatePackage` Schema，支持任意行业的部门、角色、业务字段、视图、关系、事件、成果、员工交接流程、自动化和展示配置。
- [x] 注册通用、软件研发、内容创作、长篇小说四个内置包；小说概念只存在于小说包，软件包用需求、模块、API、风险和发布计划验证通用性。
- [x] 系统级公司模板架构师 Skill 只生成结构化蓝图；一次修复失败后安全回退内置模板，不生成 HTML、脚本或新 Skill。
- [x] 创建向导由服务端模板目录驱动，先生成并展示六模块 HTML 审阅，再由用户确认创建；取消一键跳过确认。
- [x] 持久化模板定义、不可变版本、公司安装快照，以及具体到员工/字段的能力和 Skill 绑定。
- [x] Task 按明确 Skill、能力和业务字段目标解析上下文，说明加载原因，遗漏无关员工 Skill，并保留旧公司兼容回退。
- [x] 公司设置提供模板运行健康发现、稳定指纹去重、忽略/解决状态和配置入口。
- [ ] 通用业务记录实例、关系/事件编辑器、模板升级差异与用户覆盖合并，继续按独立后续计划实现。

验收命令以本轮最终执行结果为准：`npm run typecheck`、全量 `npm test -- --run`、`npm run test:product-acceptance`、`npm run build`、Playwright E2E 和 `git diff --check`。

## 2026-07-13 公司、员工与运行闭环打磨

本轮将平台从“能力已经很多”收敛为可发现、可解释、可恢复的成品主流程：

- 公司创建为模板、团队、执行器与权限、首项目、首项目任务五步向导；通用、软件研发、内容创作和长篇小说都是完整模板。
- 公司页拆为概览、团队、项目、活动、设置五个标签；驾驶舱只读取真实审批、执行器健康、项目和岗位覆盖数据，并给出唯一推荐行动。
- 员工招募统一为复用档案、新建档案和岗位模板三种来源；员工页明确拆分身份与能力、公司任职、项目工作状态。
- 任职健康由固定 Profile、最近基础联通探测、权限策略和审批桥共同判定，Profile 存在不等于可运行。
- `RunWatchdog` 覆盖启动、无输出、总时长和连续网络错误；审批在线等待、超时失败关闭、重启拒绝与持久化恢复不再隐藏卡死。
- `SessionManager` 处理软阈值压缩、硬阈值换代和有限恢复链；项目任务、审批、压缩、换代、恢复和 Watchdog 事件实时刷新对应 UI。
- 所有用户可见关键状态提供中文原因、影响和修复入口；归档说明只读影响，390px 布局和键盘标签页完成无障碍收口。
- `npm run test:product-acceptance` 提供无真实 CLI 请求的模板创建、固定绑定、健康判定、工作单和归档边界验收。

本轮最终代码级验收（2026-07-13）：

- `npm test`：67 个测试文件、346 项测试通过。
- `npm run typecheck`：通过。
- `npm run build`：server 与 client 生产构建通过。
- `npm run test:product-acceptance`：通过。
- `npm run test:e2e`：Chromium 14/14 通过。
- `git diff --check`：通过。
- 按用户要求，本轮不执行 Codex、Claude、Antigravity 的真实模型请求；这不影响上述代码级验收结论。

当前验收以本节和 `CLAUDE.md` 为准；下方带日期的旧测试数字及“待后续”描述是历史推进记录，不代表当前缺口。

> ⚠️ 2026-08-16 公司退场注记：本节 2026-07-15 模板平台/公司向导交付物已随公司退场删除（模板表已 drop）；「通用业务记录实例、关系/事件编辑器、模板升级差异」（未勾选项）随平台删除作废。员工/记忆/执行器/权限等章节仍为现行服务端域。
...
- `npm test`：当时的完整 Vitest 门禁通过。
- `npm run typecheck`：通过。
- `npm run build`：server 与 client 生产构建通过。
- `npm run test:e2e`：当时的 Chromium 门禁通过。
- `git diff --check`：通过。

当时记录的执行器、审批、模板与能力分享基础现均已纳入当前实现；剩余扩展以本文件顶部当前验收为准。
...
- 新增持久化总工作区，可创建、切换唯一激活目录；项目默认落入 `{workspace}/companies/{公司}/projects/{项目-ID}`。
- 首次主流程固定为“创建公司 → 组建团队 → 创建项目 → 发布 Task”，公司在线时仍可创建项目，完成建司后不再停在公司管理页。
- 首页、公司页、项目页统一为状态相关的单一“下一步”；项目页显示首个 Task 与发布入口。
- 第二次打开可继续最近项目；项目路由保留所属公司导航，只在高级信息中显示本地根目录。
- 设置页首屏仅保留常用连接操作，高级 CLI/权限/Provider 参数折叠；长路径与 390px 窄屏不产生横向溢出。
- 设置助手降级只返回稳定、可行动的用户提示；CLI 路径、参数、schema 与 stderr 仅写服务端日志。

验证证据：

- `npm test`：34 个测试文件、246 项测试通过。
- `npm run typecheck`：通过。
- `npm run build`：server 与 client 生产构建通过。
- `npm run test:e2e`：Chromium 9/9 通过。
- `git diff --check`：通过。

明确待后续计划实现：Agent Profile 与分层记忆、Codex/自定义 CLI Manifest、审批策略与 Turbo 范围、能力与公司模板平台、去记忆分享包。
...
依据"做完"指令，把上一轮剩余项全部推到完成。历史验证证据：`npm run typecheck`、`npm test` 与 `npm run build` 均通过。
