...
- 公司页增加驾驶舱聚合：在线员工、运行项目、执行器问题和唯一推荐行动。
- 公司向导平台化，提供通用、软件研发、内容创作与长篇小说模板；小说回归为内置模板。
- 招募入口统一解释为员工库复用、快速新建档案和智能岗位向导。
- 员工档案页明确区分全局身份、公司任职以及执行器/权限健康。
- 增加 `RunWatchdog`，与负责上下文容量的 `SessionManager` 分离。
- 项目任务和审批决定通过实时事件精确刷新前端缓存。
