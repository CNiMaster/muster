...
export function ProjectLaunchGate({ task, discovering, confirming, onDiscover, onConfirm }: {
  ...
  return <Card className="project-launch-gate" title="先确认想要的效果与可执行能力" actions={<Badge tone={task.launchState === 'draft' ? 'warn' : 'info'}>{task.launchState === 'draft' ? '待澄清' : '待确认'}</Badge>}>
    <p className="muted">先把效果、边界和资料说清楚。Muster 会检查员工固定执行器、公司能力绑定、可加载 Skill 与启用工具；公共工具目录不会被当作安装保证或强制方案。</p>
    <div className="project-launch-grid">
      ...
      <Field label="需要核验的能力（逗号分隔）" hint="使用公司已有 capability 标识；留空表示先由负责人判断。"><Input value={brief.requiredCapabilityIds.join(', ')} onChange={(event) => set({ requiredCapabilityIds: splitComma(event.target.value) })} placeholder="例如：video-editing, presentation-design" /></Field>
