...
export function EmployeeRuntimePanel({ runtime }: { runtime: EmployeeRuntimeDTO }): React.ReactElement {
  if (runtime.totals.threads === 0) return <EmptyState title="还没有项目运行记录" hint="员工首次收到项目任务中的工作单后，Muster 才会按需创建独立会话。" />;
  return <div className="section-stack">
    <Card title="运行概览"><p>{runtime.totals.projects} 个项目 · {runtime.totals.threads} 个任务会话 · {runtime.totals.workOrders} 个员工工作单 · {runtime.totals.artifacts} 项成果</p></Card>
