...
export function EmployeeRuntimePanel({ runtime }: { runtime: EmployeeRuntimeDTO }): React.ReactElement {
  ...
  return <div className="section-stack">
    ...
    {runtime.employments.flatMap((employment) => employment.projects.map((project) => <Card key={`${employment.employeeId}:${project.projectId}`} title={<Link to={`/projects/${project.projectId}`}>{project.projectName}</Link>} actions={<Badge tone="info">{employment.role}</Badge>}>
