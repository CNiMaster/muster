 ...
 *
 * 回答"设置不够智能"：不铺 20 个折叠区，先给一张 checklist（执行器/权限/工作台/上线），
...
export function SetupChecklist(): React.ReactElement | null {
  ...
  const items: Array<{ ok: boolean; label: string; href: string; action?: string }> = [
    ...
    { ok: active.length > 0, label: '工作台（选模板一键开跑）', href: '/companies/wizard', action: '去创建' },
    { ok: online.length > 0, label: '工作台上线（agent 开始工作）', href: '/companies', action: '去启动' },
  ];
  ...
  return (
    <Card title={<>就绪清单 <Badge tone={done >= 2 ? 'ok' : 'warn'}>{done}/{items.length}</Badge></>}>
      ...
      <p className="muted">以上就绪后，选一个工作台模板点「一键开跑」，就能开始和团队对话派活。</p>
    </Card>
  );
}
