...
export function TasksPage(): React.ReactElement {
  ...
  const submit = (): void => {
    ...
    createTask.mutate({
      ...
      onSuccess: () => {
        toast('success', assignee ? '员工工作单已派发' : '工作单已发布到任务池，由第一负责人领取');
  ...
  return <div className="tasks-page employee-task-board">
    ...
    <Card title="发布标准工作单" className="section compact-dispatch-card">
      <p className="muted">不必先与负责人对话。填写完整交接信息后，可直接指定员工，或发布到任务池由第一负责人领取并继续派发。</p>
      <div className="task-dispatch-grid task-work-order-grid">
        ...
        <Field label="预期交付物" required><Textarea value={deliverables} onChange={(event) => setDeliverables(event.target.value)} placeholder="员工应提交哪些文件、结论、测试或说明？" /></Field>
