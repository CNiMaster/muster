...
export function ExecutorCenterPage():React.ReactElement{
  ...
  const[connectionResults,setConnectionResults]=useState<Record<string,{version:{success:boolean;output:string};authentication:{supported:boolean;success:boolean;output:string}}>>({});
  ...
  const testConnection=useMutation({mutationFn:(id:string)=>api.post<{version:{success:boolean;output:string};authentication:{supported:boolean;success:boolean;output:string}}>(`/api/executors/profiles/${id}/test`),onSuccess:(result,id)=>setConnectionResults((old)=>({...old,[id]:result})),onError:(e:any)=>toast('error',e.message??'连接诊断失败')});
