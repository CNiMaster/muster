...
export function HomePage(): React.ReactElement {
  ...
  const primaryProject = (projects ?? []).find((p) => p.state === 'active') ?? projects?.[0];
  const { data: primaryProjectTasks } = useProjectTasks(primaryProject?.id);
  const { data: primaryTasks } = useTasks(primaryProject?.id);
  const attentionTotal = (primaryTasks?.filter((t) => t.state === 'blocked' || t.state === 'waiting_input').length ?? 0) + (cockpit?.approvals.pending ?? 0);
  ...
  return (
    <WorkbenchShell
      scopeKey="workbench:overview"
      breadcrumb={
        <WorkbenchContextSwitcher
          projectId={primaryProject?.id}
          projectName={primaryProject?.name ?? '工作台'}
          sectionKey="dashboard"
          sectionLabel="全部项目与任务"
          novel={company?.kind === 'novel'}
        />
      }
      navigationLabel="项目组织与联系人"
      inspectorLabel="工作台运行与关注"
      ...
      primaryAction={
        <>
          ...
          <DropdownMenu
            ...
            buttonClassName="mu-btn mu-btn-primary mu-btn-sm workbench-publish-action"
          >
            ＋ 新建项目
          </DropdownMenu>
        </>
      ...
      navigation={
        <ProjectWorkNavigation
          ...
          attentionCount={attentionTotal}
          novel={company?.kind === 'novel'}
          onNewTask={() => navigate(primaryProject ? `/projects/${primaryProject.id}?view=task&projectTask=new` : '/projects/new')}
        />
      }
      inspector={
      ...
      commandOptions={[
        ...(projects ?? []).slice(0, 5).map((p) => ({ label: `项目：${p.name}`, href: `/projects/${p.id}`, group: '项目列表' })),
        ...(agents ?? []).slice(0, 5).map((agent) => ({ label: `智能体：${agent.name}`, href: primaryProject ? `/projects/${primaryProject.id}?view=employee&agent=${agent.id}` : '/agents', group: '团队成员' })),
        { label: '智能体人才库', href: '/agents', group: '全局功能' },
        { label: '蓝图库', href: '/blueprints', group: '全局功能' },
        { label: '归档中心', href: '/archive', group: '全局功能' },
        { label: '自动化中心', href: '/automations', group: '全局功能' },
        { label: '系统设置', href: '/settings', group: '全局功能' },
      ]}
    >
      <div className="home-page work-surface-page" style={{ width: '100%', maxWidth: '1050px', margin: '0 auto', padding: '4px 0 40px' }}>
        {/* ===== 对话开工（紧凑 hero） ===== */}
        <section style={{ padding: '12px 10px 6px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <h1 style={{ fontSize: 20, fontWeight: 750, margin: 0 }}>你想开始什么新工作？</h1>
            <DropdownMenu
              label="新建项目"
              items={[
                { key: 'new', label: '🆕 新建项目', onSelect: () => navigate('/projects/new') },
              ...
              buttonClassName="mu-btn mu-btn-primary mu-btn-sm"
            >
              ＋ 新建项目
            </DropdownMenu>
          </div>
        ...
        {/* ===== 任务时间线（视图=时间线时） ===== */}
        {taskView === 'timeline' && (
          <section style={{ padding: '12px 10px 0' }}>
            <TimelineTasks projects={projects ?? []} sortMode={sortMode} />
          </section>
        )}

        {/* ===== 项目区 + 任务区 ===== */}
        <section style={{ padding: '12px 10px 40px' }}>
          <StandaloneSection />
