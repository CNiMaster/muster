-- safety: rebuild
-- capability parity 批次 D4：plugin 重建扩 kind CHECK 加 'hook'（生命周期钩子）。
-- 沿用 20260823120000（panel）同款重建模式（SQLite CHECK 不可原位修改）。
CREATE TABLE plugin_new (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  kind TEXT NOT NULL CHECK (kind IN ('skill', 'mcp-server', 'tool', 'bridge-action', 'ai-generated', 'panel', 'hook')),
