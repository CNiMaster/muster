...
export const BUILTIN_EXECUTOR_MANIFESTS: readonly Readonly<ExecutorManifest>[] = deepFreeze([
  ...
  { id: 'pi-cli', version: 1, displayName: 'pi CLI', kind: 'cli', certification: 'experimental', legacy: false, approvalBridge: 'none', limitations: ['无原生审批接管：pi 无 permission hook（设计哲学=容器/扩展自建），依赖 L0 OS 围栏（seatbelt 文件写白名单）+ 可选 --tools 只读白名单——与 custom CLI 同类已知边界', 'JSON 事件流依赖 pi ≥0.73（@mariozechner/pi-coding-agent）', 'provider/model 走 pi 自身配置（~/.pi/agent 或 --api-key 注入），muster 侧凭据解析经 --api-key 透传'], officialSource: 'https://github.com/earendil-works/pi', platforms: ['darwin', 'linux', 'win32'], detection: { command: 'pi', args: ['--version'] }, minimumVersion: null, session: { create: true, resume: true, fork: true, compact: false, abort: false }, isolation: { cwd: true, configDir: true, tempDir: true }, permissions: { scoped: false, turboFlag: null }, concurrency: 'parallel', officialInstall: { guideUrl: 'https://pi.dev', commands: ['npm install -g @mariozechner/pi-coding-agent'], binaryName: 'pi', loginCommand: 'pi' }, defaultCapabilities: ['code'] },
