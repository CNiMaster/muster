...
export async function runConnectionProbe(db:DB,input:{profileId:string;force:boolean;kind?:ProbeKind;runtime?:ProbeRuntime;now?:Date;probeId?:string}):Promise<ConnectionProbe> {
  ...
  // capability 探针也按 model 缓存（不同 model 能力不同）
  const model=(kind==='model'||kind==='capability')&&typeof profile.config.model==='string'?profile.config.model:undefined;
  if(kind==='model'&&!model)throw new Error('执行器未配置指定模型');
  const cacheKey=JSON.stringify([kind,binary,version,model??null,'login-shell']); const now=input.now??new Date();
