...
export function assertProjectTaskActive(db:DB,id:string,projectId?:string):ProjectTask{const task=getProjectTask(db,id);if(projectId&&task.projectId!==projectId)throw new AppError(ErrorCode.VALIDATION,'项目任务不属于当前项目');if(task.state==='archived')throw new AppError(ErrorCode.CONFLICT,'项目任务已归档，不能继续派工');return task;}
export function completeProjectTask(db:DB,id:string):ProjectTask{const task=assertProjectTaskActive(db,id);const now=nowIso();db.prepare("UPDATE project_task SET state='completed',completed_at=?,updated_at=? WHERE id=?").run(now,now,task.id);return getProjectTask(db,id);}
export function archiveProjectTask(db:DB,id:string):ProjectTask{const task=getProjectTask(db,id);if(task.state==='archived')return task;const now=nowIso();db.transaction(()=>{db.prepare("UPDATE task SET state='cancelled',lease_owner_thread_id=NULL,lease_expires_at=NULL,heartbeat_at=NULL,updated_at=? WHERE project_task_id=? AND state NOT IN ('completed','failed','cancelled')").run(now,id);db.prepare("UPDATE project_task_thread SET state='archived',updated_at=? WHERE project_task_id=?").run(now,id);db.prepare("UPDATE project_task SET state='archived',archived_at=?,updated_at=? WHERE id=?").run(now,now,id);})();return getProjectTask(db,id);}
