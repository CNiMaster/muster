...
executorsRouter.post('/profiles/:id/probes',asyncHandler(async(req,res)=>{const input=z.object({force:z.boolean().default(false),kind:z.enum(['connectivity','model']).default('connectivity')}).parse(req.body??{});res.status(202).json(startConnectionProbe(getDb(),{profileId:param(req,'id'),force:input.force,kind:input.kind}));}));
