...
executorsRouter.put('/profiles/:id', asyncHandler(async (req,res)=>{
  const input = z.object({
    ...
    contextWindowTokens: z.number().int().min(1).nullish(),
  }).parse(req.body);
...
executorsRouter.post('/profiles/:id/probes',asyncHandler(async(req,res)=>{const input=z.object({force:z.boolean().default(false),kind:z.enum(['connectivity','model','capability']).default('connectivity'),model:z.string().min(1).optional()}).parse(req.body??{});res.status(202).json(startConnectionProbe(getDb(),{profileId:param(req,'id'),force:input.force,kind:input.kind,...(input.model?{modelOverride:input.model}:{})}));}));
