...
projectById.post('/project-tasks',asyncHandler(async(req,res)=>{const input=z.object({title:z.string().min(1),brief:z.string().optional(),launchBrief:projectLaunchBriefSchema.optional(),blueprintId:z.string().optional()}).parse(req.body);const projectId=param(req,'id');const launchBrief={...(input.launchBrief??projectLaunchBriefSchema.parse({})),blueprintId:input.blueprintId||input.launchBrief?.blueprintId||''};const task=createProjectTask(getDb(),{projectId,title:input.title,brief:input.brief,launchState:'draft',launchBrief});realtime.publish(makeLifecycleEvent('project-task.created',{projectTaskId:task.id},{projectId}));res.status(201).json(task);}));
