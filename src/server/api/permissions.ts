...
permissionsRouter.post('/approvals/:id/decision', asyncHandler(async (req,res)=>{ const input=z.object({decision:z.enum(['deny','allow-once','allow-command','allow-directory','custom-rule']),rule:z.any().optional()}).parse(req.body); recordApprovalDecision(getDb(),param(req,'id'),input); res.json({ok:true}); }));
