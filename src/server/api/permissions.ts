...
permissionsRouter.post('/approvals/:id/decision', asyncHandler(async (req,res)=>{ const input=z.object({decision:z.enum(['deny','allow-once','allow-command','allow-directory','custom-rule']),rule:z.any().optional()}).parse(req.body);const id=param(req,'id');const online=approvalBroker.hasWaiter(id);recordApprovalDecision(getDb(),id,input,{resumeTask:!online});approvalBroker.resolve(id,input.decision==='deny'?'deny':'allow');realtime.publish({id:`ev_${Date.now()}`,type:'approval.decided',occurredAt:new Date().toISOString(),payload:{approvalId:id,decision:input.decision,online}}); res.json({ok:true}); }));
