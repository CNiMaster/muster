...
从「我有件事要办」一句话直接开工——不用先设计组织、不用挑模型配岗位。打法存在**蓝图**里：什么类型的活 → 配什么班底 → 用什么工具 → 战绩如何。开箱自带 **26 套预制蓝图**（软件交付 / 长篇小说 / 内容写作 / 调研咨询 / 营销推广 / 数据可视化 / 网站开发 / Agent 应用 / 幻灯片演示…覆盖创作、工程与办公场景），建「写一章小说」的任务即刻穿戴主笔+主编+审校班底；原版永远保留，用着不对可一键重置。
...
| **专家穿戴** | 240 个嵌入式 AI 人设按任务穿戴，自带领域知识包——同一智能体，写小说时是主笔，修 bug 时是架构师 |
...
- **蓝图阶段流水线**：蓝图可把一类活拆成阶段流水线——串行推进、阶段门自检或验收员快评、缺人自动改派；画布上连线调班底与工具绑定，保存即生效。
- **自动化中心**：12 套开箱模版（每日新闻推送 / 工作周报 / 家人联系提醒…），定时或条件触发，到期系统提醒并可自动派发任务。
- **执行器平台**：五款 CLI（Claude Code / Codex / OpenCode / Antigravity / pi）一键检测/安装/AI 诊断，另支持自定义 CLI 与 OpenAI 兼容、Gemini API 凭据接入，档位降级链自动兜底。
