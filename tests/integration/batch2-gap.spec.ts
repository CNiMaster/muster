...
describe('B2.2 时间·里程碑触发复盘', () => {
  ...
  it('已有 open 复盘时不再触发', () => {
    const c = createCompany(db, { name: 'co' });
    const lead = createAgent(db, { companyId: c.id, name: 'lead', role: 'lead' });
    const p = createProject(db, { companyId: c.id, name: 'p', rootDir: '/tmp/p4', firstAgentId: lead.id });
...
describe('B2.3 多模型 token 归集 + 公司级聚合', () => {
  ...
  it('summarizeCompanyUsage 跨项目聚合', () => {
    const c = createCompany(db, { name: 'co' });
    const lead = createAgent(db, { companyId: c.id, name: 'lead', role: 'lead' });
    const p1 = createProject(db, { companyId: c.id, name: 'p1', rootDir: '/tmp/p6a', firstAgentId: lead.id });
    const p2 = createProject(db, { companyId: c.id, name: 'p2', rootDir: '/tmp/p6b', firstAgentId: lead.id });
...
describe('B2.4 讨论结论→建议 Task', () => {
  it('brainstorm 完成后创建建议 Task，claimNextTask 不领取建议', () => {
    const c = createCompany(db, { name: 'co' });
    const lead = createAgent(db, { companyId: c.id, name: 'lead', role: 'lead' });
    const p = createProject(db, { companyId: c.id, name: 'p', rootDir: '/tmp/p7', firstAgentId: lead.id });
  ...
  it('acceptSuggestion 清除标记并恢复优先级', () => {
    const c = createCompany(db, { name: 'co' });
    const lead = createAgent(db, { companyId: c.id, name: 'lead', role: 'lead' });
    const p = createProject(db, { companyId: c.id, name: 'p', rootDir: '/tmp/p8', firstAgentId: lead.id });
...
describe('B2.soak 连续 Task 稳定性 minitest', () => {
  it('连续创建 50 个 Task 并完成，无重复 seq，无丢任务', () => {
    const c = createCompany(db, { name: 'co' });
    const lead = createAgent(db, { companyId: c.id, name: 'lead', role: 'lead' });
    const p = createProject(db, { companyId: c.id, name: 'p', rootDir: '/tmp/p9', firstAgentId: lead.id });
    const ids: string[] = [];
    for (let i = 0; i < 50; i++) {
