...
describe('RuntimeToolRegistry 基础', () => {
  it('createBuiltinToolRegistry 注册 31 个内置工具', () => {
    const reg = createBuiltinToolRegistry();
    expect(reg.size()).toBe(31);
    const names = reg.definitions().map((d) => d.function.name).sort();
    expect(names).toEqual(
      ['ask_colleague', 'cancel_child_task', 'conclude_discussion', 'document_append', 'document_create', 'done', 'edit_file', 'glob_files', 'host_knowledge_append', 'host_knowledge_query', 'host_plugin_list', 'host_plugin_toggle', 'host_settings_get', 'host_settings_set', 'image_generate', 'list_files', 'notify_colleague', 'notify_host', 'read_file', 'read_persona_manual', 'run_command', 'search_files', 'search_knowledge', 'spawn_tasks', 'start_discussion', 'submit_review', 'todo_read', 'todo_write', 'web_fetch', 'web_search', 'write_file'],
    );
  });
