...
describe('RuntimeToolRegistry 基础', () => {
  it('createBuiltinToolRegistry 注册 13 个内置工具', () => {
    const reg = createBuiltinToolRegistry();
    expect(reg.size()).toBe(14);
    const names = reg.definitions().map((d) => d.function.name).sort();
    expect(names).toEqual(
      ['ask_colleague', 'cancel_child_task', 'conclude_discussion', 'done', 'edit_file', 'list_files', 'notify_colleague', 'notify_host', 'read_file', 'run_command', 'spawn_tasks', 'start_discussion', 'submit_review', 'write_file'],
    );
  });
