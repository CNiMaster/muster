 ...
 * 实测迁移链终态 schema 已是 ('personal','workspace','project','skill')——前提不成立（指认读的是 0017 基础文件而非终态），
...
describe('memory scope CHECK 守卫', () => {
  it('全部四个 scope 枚举值均可落库（TS 枚举与 DB CHECK 不漂移）', () => {
    ...
    for (const scope of ['personal', 'workspace', 'project', 'skill'] as const) {
      ...
      const candidate = createMemoryCandidate(db, {
        ...
        ...(scope === 'skill' ? { personaKey: 'product/manager' } : {}),
        content: `守卫：${scope} 落库`,
        author: 'agent',
        confidence: 1,
        canInfluence: true,
    ...
    expect(rows.map((r) => r.scope).sort()).toEqual(['personal', 'project', 'skill', 'workspace']);
  });
});
