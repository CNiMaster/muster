...
const findings: TemplateRuntimeHealthFinding[] = [{
  ...
  message: '需求维护绑定的 Skill 没有安装。', impact: '智能体仍能执行，但方法稳定性会下降。',
  cause: '模板引用 missing-skill，但本机找不到对应能力。', recommendation: '前往智能体能力设置替换或移除。',
  action: { kind: 'open_employee', label: '配置智能体能力', href: '/companies/co_1?view=team' },
...
describe('template health panel', () => {
  it('explains the issue, impact, cause, recommendation, and guided action', async () => {
    ...
    expect(screen.getByRole('link', { name: '配置智能体能力' })).toHaveAttribute('href', '/companies/co_1?view=team');
  ...
  it('shows a clear healthy state', () => {
    ...
    expect(screen.getByText('工作台模板运行正常')).toBeVisible();
  });
});
