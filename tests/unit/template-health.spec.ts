...
describe('company template registry', () => {
  it('registers four schema-valid built-in packages with industry knowledge models', () => {
    expect(listBuiltinCompanyTemplates().map((template) => template.id)).toEqual(['general', 'software', 'content', 'novel', 'marketing', 'consulting', 'visual', 'video', 'publishing', 'social']);
